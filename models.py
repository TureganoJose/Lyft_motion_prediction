...
def pytorch_neg_multi_log_likelihood_batch(gt: Tensor, pred: Tensor, confidences: Tensor, avails: Tensor) -> Tensor:
    ...
    # error (batch_size, num_modes)
    max_value, _ = error.max(dim=1,
                             keepdim=True)  # error are negative at this point, so max() gives the minimum one
...
class LyftMultiModel_carstates(nn.Module):
    ...
    def __init__(self, cfg: Dict, num_modes=3):
        ...
        num_carstates = 4
        total_features = num_carstates + backbone_out_features
        ...
        self.head = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(in_features=total_features, out_features=4096),
        )
    ...
    def forward(self, x, car_states):
        ...
        x = torch.cat([x, car_states], dim=1)
        x = self.head(x)
        x = self.logit(x)
