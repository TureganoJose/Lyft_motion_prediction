...
from agent_sampling_experiments import custom_generate_agent_sample
...
None if not desired
    ...
    def get_frame(self, scene_index: int, state_index: int, label: int, track_id: Optional[int] = None) -> dict:
        """
...
class CustomAgentDataset(CustomEgoDataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        return self.get_frame(scene_index, state_index, agent_label, track_id=track_id) #.update({'label': agent_label})

    def get_agent_labels(self):
        return self.agents_category
