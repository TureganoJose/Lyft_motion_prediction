...
from torchvision.models.resnet import resnet18, resnet50, resnet101, resnet34
...
class decoder(nn.Module):
    def __init__(self,  cfg: Dict,  input_size, hidden_size, n_layers=1, drop_prob=0, n_frame_history=11, batch_size=32,
        ...
        self.logit = nn.Linear(n_frame_history*hidden_size+4096, out_features=self.num_preds + num_modes)


    def forward(self, inputs, hidden, map_predictor):
        ...
        x = torch.cat((map_predictor, decoder_lstm_output), dim=1)
        x = self.logit(x)
...
class STAMINA_res_net(nn.Module):
    def __init__(self, cfg: Dict, n_agents=100, n_car_states=7, n_frames=11, batch_size=32, device='cpu',
        ...
        super(STAMINA_res_net, self).__init__()
        # General parameters
        self.n_agents = n_agents
        self.n_car_states = n_car_states
        ...
        # Resnet
        self.map_layer = LyftMultiModel(cfg)
    ...
    def forward(self, input_data):
        ...
        map_predictor = self.map_layer(input_image)

        h_final = self.final_layer.init_hidden(self.batch_size)
        predictions, probabilities = self.final_layer(final_encoded_tensor.to(self.device), h_final, map_predictor)
