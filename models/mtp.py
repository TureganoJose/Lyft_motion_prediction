...
from models.backbone import calculate_backbone_feature_dim
...
class MTPLoss:
    ...
    def __call__(self, predictions: torch.Tensor, targets: torch.Tensor, target_mask: torch.Tensor) -> torch.Tensor:
        """
        ...
        for batch_idx in range(predictions.shape[0]):
            ...
            # MSE loss applying target mask
            n_valid_targets = torch.sum(target_mask[batch_idx])
            l2_norm = torch.norm(best_mode_trajectory- targets[batch_idx], dim=2) * target_mask[batch_idx]
            regression_loss = torch.sum(l2_norm)/n_valid_targets
